...
/**
 * IME 输入法自动切换控制器
 * 基于 Shell Hook 事件驱动，监听前台窗口切换并自动管理输入法状态
 * 与主循环完全解耦，不干扰 CATIA 热键系统
 */
class IMEController {

    /**
     * 初始化 Shell Hook 事件监听
     * 利用脚本自身的隐藏窗口接收系统窗口激活消息
     */
    static Init() {
        DllCall("RegisterShellHookWindow", "Ptr", A_ScriptHwnd, "Int")
        shellMsg := DllCall("RegisterWindowMessage", "Str", "SHELLHOOK", "UInt")
        OnMessage(shellMsg, ObjBindMethod(this, "OnWindowChange"))
    }

    /**
     * 窗口激活事件回调（无状态）
     * 进入 autoime 组 → 切英文；离开 autoime 组 → 切中文
     * switchIMEbyID 基于 PostMessage，已是目标语言时等效空操作
     * 
     * @param wParam  4 = HSHELL_WINDOWACTIVATED, 0x8004 = HSHELL_RUDEAPPACTIVATED
     */
    static OnWindowChange(wParam, lParam, msg, hwnd) {
        if (wParam != 4 && wParam != 0x8004)
            return

        try {
            if WinActive("ahk_group group_autoime") {
                if AppSettings.AutoIME_Enable {
                    switchIMEbyID(IMEmap["en"])
                    SetTimer(confirmIME, -5000)
                }
            } else if (AppSettings.AutoIME_RestoreChinese) {
