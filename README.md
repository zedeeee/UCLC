...
8. **快捷弹窗操作**：在 CATIA 弹窗（如拉伸、开槽等）中，通过 `Alt + 鼠标中键` 快捷点击“确定/OK”，`Shift + 鼠标中键` 快捷点击“预览/Preview”，极大减少鼠标移动距离。（*注：本功能目前仅在 CATIA V5R27 上测试通过*）
...
> [!WARNING]
> **配置迁移说明 (v2.5.0+)**  
> 自 v2.5.0 起，配置文件新增了弹窗快捷键功能。请勿使用旧版 `config.ini` 直接覆盖。请在新的 `config.ini` 中找到 `[DevFunc]` 下的 `MBUTTON_DIALOG_SHORTCUTS = 1` 来启用中键弹窗快捷功能，或通过系统托盘菜单的“开发功能”进行切换。
